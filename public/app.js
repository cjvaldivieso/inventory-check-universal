/* public/app.js — Shappi Inventory App (v18 final, FIXED + Battery Saver)
   Contains:
...
// --------------------------------------------------
// Decode fewer times per second (instead of every animation frame)
const DECODE_INTERVAL_MS = 200; // ~5 decodes/sec
// Downscale the frame before jsQR (big battery/CPU saver)
const DECODE_MAX_WIDTH   = 640;
// Lower FPS reduces power draw on iPhone/Android
const CAMERA_FPS_IDEAL   = 15;

// Get camera stream with power-friendly constraints
...
// Draw a downscaled frame into canvas and return ImageData for jsQR
function getDownscaledImageData(video, canvas, ctx) {
  ...
  try {
    ...
    const loop = async () => {
      ...
      if (video.readyState === video.HAVE_ENOUGH_DATA) {
        ...
        // Throttle decode (battery saver)
        if (now - lastDecode >= DECODE_INTERVAL_MS) {
          lastDecode = now;

          const frame = getDownscaledImageData(video, canvas, ctx);
          if (frame) {
      ...
      if (video.readyState === video.HAVE_ENOUGH_DATA) {
        ...
        if (now - lastDecode >= DECODE_INTERVAL_MS) {
          ...
          if (frame) {
            ...
            // Keep your existing item debounce
